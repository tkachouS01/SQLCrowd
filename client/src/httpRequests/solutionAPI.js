...
export const createComment = async (contextUser, contextTask, contextSolution, solutionId, content, navigate) => {
   let result = false;
    ...
    await $authHost.post(`${baseUrlApi}/tasks/${contextTask.currentTask._id}/solutions/${solutionId}/comment`, {content: content})
        .then(() => {
            result = getSolutions(contextUser,contextTask,contextSolution, contextTask.currentTask._id, navigate)
        })
        .catch(error => {
            contextUser.setErrorMessage(`${error.response.status}`, error.response.data.message)
        })
...
export const like = async (contextUser, contextTask, contextSolution, solutionId) => {
   let result = false;
    ...
    await $authHost.post(`${baseUrlApi}/tasks/${contextTask.currentTask._id}/solutions/${solutionId}/like`, {})
        .then(data => {
